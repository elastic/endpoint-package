...
When Elastic Defend is deployed as a Kubernetes DaemonSet, host isolation is automatically disabled and the TC probes are not installed. However, DaemonSet deployment is not officially supported. For process-based installations on Kubernetes nodes, if host isolation is not required on these nodes, set `linux.advanced.host_isolation.allowed` to `false` in the Elastic Defend advanced policy settings. This completely disables the host isolation plugin and prevents Elastic Defend from installing any TC eBPF probes, eliminating the conflict with other eBPF-based networking tools. Because this turns off host isolation, only apply it when host isolation is not needed on the affected endpoints.
