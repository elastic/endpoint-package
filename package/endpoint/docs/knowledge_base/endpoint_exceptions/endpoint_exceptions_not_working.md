...
### Endpoint Alerts require Endpoint Exceptions not SIEM exceptions

Endpoint writes alerts to `logs-endpoint.alerts-*` then SIEM runs the Defend rule(s) and "promotes" the alert into a SIEM alert in the SIEM index. If the user creates a SIEM Rule Exception then Endpoint will still block/detect the behavior on the host and write an alert to `logs-endpoint.alerts-*` but the SIEM will not promote the alert. This makes it seem like Endpoint is blocking system activity but not alerting on it.

To fix, do not use Rule Exceptions for Endpoint behavior. Only advanced users have a reason to use Rule Exceptions for the Defend rule(s).
