...
func renderReadme(options generateOptions, packageName string) ([]string, error) {
	...
	if err != nil {
		return nil, errors.Wrapf(err, "opening README file for writing failed (path: %s)", outputPath)
	}
	defer f.Close()

	err = t.Execute(f, nil)
	if err != nil {
		return nil, errors.Wrapf(err, "rendering README file failed (path: %s)", templatePath)
	}
	return dataStreams, nil
}

func renderReadmePlatform(options generateOptions, packageName, dataStream, os_ string) error {

	output, err := renderFilteredFields(options, packageName, dataStream, os_)

	if err != nil {
		return errors.Wrapf(err, "failed to render fields for %s/%s/%s", packageName, dataStream, os_)
	}
	...
	if len(output) > 0 {
		...
		if _,err := os.Stat(outputDir); errors.Is(err, os.ErrNotExist) {
			...
			if err != nil {
				return errors.Wrapf(err, fmt.Sprintf("failed to make directory %s", outputDir))
			}
		}
		...
		if err != nil {
			return errors.Wrapf(err, "rendering README file failed (path: %s)", outputPath)
		}
	}
	return nil
}
