# Linux process_vm_access Cross-process Memory Access Events

- OS: Linux
- Data Stream: `logs-endpoint.events.process-*`
- KQL: `event.action : "process_vm_access" and event.dataset : "endpoint.events.process" and event.module : "endpoint" and host.os.type : "linux"`

Diagnostic syscall attempts and results for process_vm_readv(2) and process_vm_writev(2). Failed and unresolved attempts are retained. Vector metadata is a best-effort entry snapshot, not authoritative accessed ranges. Collection requires process capture and diagnostic configuration; Elasticsearch output separately requires emit_diagnostic_fields.
...
| process.Ext.process_vm_access.bytes_transferred |
| process.Ext.process_vm_access.caller_pidns |
| process.Ext.process_vm_access.error |
| process.Ext.process_vm_access.first_remote_addr |
| process.Ext.process_vm_access.first_remote_len |
| process.Ext.process_vm_access.first_remote_valid |
| process.Ext.process_vm_access.flags |
| process.Ext.process_vm_access.local_capacity |
| process.Ext.process_vm_access.local_iovcnt |
| process.Ext.process_vm_access.local_snapshot_status |
| process.Ext.process_vm_access.operation |
| process.Ext.process_vm_access.remote_capacity |
| process.Ext.process_vm_access.remote_iovcnt |
| process.Ext.process_vm_access.remote_snapshot_status |
| process.Ext.process_vm_access.requested_capacity |
| process.Ext.process_vm_access.requested_pid |
| process.Ext.process_vm_access.syscall_return |
| process.Ext.process_vm_access.target_pid |
| process.Ext.process_vm_access.target_resolved |
| process.Ext.process_vm_access.target_start_time_ns |
