...
<ul>Indicates whether a method was called locally or remotely. It will be true if called locally, and false if called remotely.</ul>
...
<ul>Client process's machine name (provided by the client and potentially untrustworthy).</ul>

#### Example

<ul><code>DESKTOP-EXAMPLE</code></ul>
...
<ul>Client process's machine name FQDN (provided by the client and potentially untrustworthy).</ul>

#### Example

<ul><code>DESKTOP-EXAMPLE.elastic.co</code></ul>
...
<ul>Client process id</ul>

#### Example

<ul><code>3600</code></ul>
...
<ul>Provides specific information about an event consumer, including its configuration, such as the command it executes, associated SID, and the consumer's name.</ul>
...
<ul>Consumer Name. Consumers specify what should happen when the conditions in the Event Filter are triggered.</ul>

#### Example

<ul><code>ExampleConsumer</code></ul>
...
<ul>Provide an overview of the query that define when an event should be triggered.</ul>
...
<ul>Event filter name. An event filter is a WMI class that defines which events WMI delivers to a consumer.</ul>

#### Example

<ul><code>ExampleFilter</code></ul>
...
<ul>WMI namespace to which the connection is made.</ul>

#### Example

<ul><code>root\Microsoft\Windows\DeviceGuard</code></ul>
...
<ul>Specifies the connection or request to WMI</ul>

#### Example

<ul><code>Win32_Process::Create</code></ul>
