...
<ul>This field indicates a number of all GetAsyncKeyState api calls, including unsuccessful calls, between the last successful GetAsyncKeyState call.</ul>

#### Example

<ul><code>6021</code></ul>
...
<ul>This field indicates the elapsed time in milliseconds since the last GetAsyncKeyState event.</ul>
...
<ul>Summary of the hook procedure.</ul>

#### Example

<ul><code>taskbar.dll</code></ul>
...
<ul>Memory protection attributes associated with the starting address of a thread.</ul>
...
<ul>Name of the module associated with the starting address of a thread.</ul>

#### Example

<ul><code>C:\Windows\System32\DellTPad\ApMsgFwd.exe</code></ul>
...
<ul>Number of visible windows owned by the caller thread.</ul>
...
<ul>Number of windows owned by the caller thread.</ul>
...
<ul>Mode flag that specifies how to interpret the information provided by UsagePage and Usage. Third member RAWINPUTDEVICE structure.</ul>

#### Example

<ul><code>INPUTSINK</code></ul>
...
<ul>DLL containing the hook procedure.</ul>

#### Example

<ul><code>c:\windows\system32\taskbar.dll</code></ul>
...
<ul>Type of hook procedure to be installed.</ul>

#### Example

<ul><code>WH_KEYBOARD_LL</code></ul>
...
<ul>This parameter indicates the specific device (Usage) within the Usage Page. Second member RAWINPUTDEVICE structure.</ul>

#### Example

<ul><code>KEYBOARD</code></ul>
...
<ul>This parameter indicates the top-level collection (Usage Page) of the device. First member RAWINPUTDEVICE structure.</ul>

#### Example

<ul><code>GENERIC</code></ul>
