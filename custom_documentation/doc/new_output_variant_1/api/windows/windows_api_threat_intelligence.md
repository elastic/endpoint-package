...
<ul>The name of the memory region targeted by the API call.</ul>

#### Example

<ul><code>Unbacked</code></ul>
...
<ul>The path of the memory region targeted by the API call.</ul>

#### Example

<ul><code>C:\programdata\example.dll</code></ul>
...
<ul>The target memory address.</ul>
...
<ul>The type of memory allocation. Corresponds to `MEMORY_BASIC_INFORMATION.State`</ul>

#### Example

<ul><code>COMMIT|RESERVE</code></ul>
...
<ul>The first argument to the procedure.</ul>
...
<ul>The second argument to the procedure.</ul>
...
<ul>The third argument to the procedure.</ul>
...
<ul>The bitmask of CPU registers operated on by this call. Corresponds to `CONTEXT.ContextFlags`</ul>

#### Example

<ul><code>1048607</code></ul>
...
<ul>The name of the device object.</ul>

#### Example

<ul><code>\Device\NPCAP</code></ul>
...
<ul>The name of the driver object.</ul>
...
<ul>The x86 EAX general purpose register. Return value in __cdecl, __stdcall, __thiscall and __fastcall.</ul>
...
<ul>The x86 EBP frame pointer register.</ul>

#### Example

<ul><code>15006644</code></ul>
...
<ul>The x86 EBX general purpose register.</ul>
...
<ul>The x86 ECX general purpose register. First argument in __fastcall and __thiscall.</ul>
...
<ul>The x86 EDI general purpose register.</ul>
...
<ul>The x86 EDX general purpose register. Second argument in a __fastcall.</ul>
...
<ul>The x86 EIP instruction pointer register.</ul>

#### Example

<ul><code>1472790528</code></ul>
...
<ul>The x86 ESI general purpose register.</ul>
...
<ul>The x86 ESP stack pointer register.</ul>

#### Example

<ul><code>15007744</code></ul>
...
<ul>This parameter indicates whether the detected access was attempt against a process or a thread.</ul>
...
<ul>The memory address of the procedure or function.</ul>
...
<ul>The memory protection for the region of pages. Corresponds to `MEMORY_BASIC_INFORMATION.Protect`</ul>

#### Example

<ul><code>RWX|GUARD</code></ul>
...
<ul>The previous memory protection returned by the API call. Corresponds to `MEMORY_BASIC_INFORMATION.Protect`</ul>
...
<ul>The x64 R8 general purpose register. Third argument in a __fastcall.</ul>
...
<ul>The x64 R9 general purpose register. Fourth argument in a __fastcall.</ul>
...
<ul>The x64 RAX general purpose register. Return value in a __fastcall.</ul>
...
<ul>The x64 RBP general purpose register.</ul>
...
<ul>The x64 RBX general purpose register.</ul>
...
<ul>The x64 RCX general purpose register. First argument in a __fastcall.</ul>
...
<ul>The x64 RDI general purpose register.</ul>
...
<ul>The x64 RDX general purpose register. Second argument in a __fastcall.</ul>
...
<ul>The x64 RIP instruction pointer register.</ul>

#### Example

<ul><code>140706153693184</code></ul>
...
<ul>The x64 RSI general purpose register.</ul>
...
<ul>The x64 RSP stack pointer register.</ul>
